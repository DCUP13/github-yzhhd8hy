/*
# Fix apply_settings_group_to_account for autoresponder unique constraint

The instagram_autoresponder_settings table has UNIQUE(account_id).
When sharing an autoresponder to a target account that already has its own
autoresponder setting (without a group), the insert fails.

Fix: check for ANY existing autoresponder setting for the target account,
not just one with the same settings_group_id. If one exists, update it
...
BEGIN
  ...
  IF v_group.setting_type = 'flow' THEN
    -- Find the source flow (any flow in the group, prefer the original/oldest)
  ...
  ELSIF v_group.setting_type = 'rule' THEN
    ...
    -- Check for existing copy with same group
