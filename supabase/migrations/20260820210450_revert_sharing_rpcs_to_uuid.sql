/*
# Revert sharing RPCs back to uuid params

## Problem
Changing params from uuid to text caused a different error: "function apply_settings_group_to_account(uuid, uuid) does not exist".
This is because PostgREST's schema cache was stale and the text-params approach didn't resolve the original issue.
The original "cannot cast jsonb to uuid" error is better fixed by ensuring the frontend sends the RPC correctly.

## Fix
Revert all four sharing functions back to uuid parameters (the correct type).
PostgREST handles uuid params fine when the JSON body contains valid UUID strings.
The original error was likely caused by one of the passed values being undefined/null
or not a valid UUID string, which makes PostgREST unable to cast.

## Changes
1. Recreate apply_settings_group_to_account with uuid params
2. Recreate unsubscribe_account_from_group with uuid params
3. Recreate resubscribe_account_to_group with uuid params
4. Recreate share_all_settings_to_account with uuid params
5. Re-grant EXECUTE to authenticated
*/

-- apply_settings_group_to_account
...
-- unsubscribe_account_from_group
