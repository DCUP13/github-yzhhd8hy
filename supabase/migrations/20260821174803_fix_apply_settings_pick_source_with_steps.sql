/*
# Fix: apply_settings_group_to_account picks a source flow with steps

## Problem
When re-syncing settings between accounts, `apply_settings_group_to_account`
picks the "source" flow using `ORDER BY created_at ASC LIMIT 1`. If the oldest
flow in the group has 0 steps (e.g. it was created as an empty synced copy
moments before the original), it becomes the source and wipes the steps from
all other copies — the flow appears empty in the UI.

## Fix
Change the source-flow selection to prefer a flow that has at least one step.
Only fall back to the oldest flow if none in the group have steps.

## Scope
- Modifies function `apply_settings_group_to_account` (replaces the function body)
...
BEGIN
  ...
  IF v_group.setting_type = 'flow' THEN
    -- Prefer a source flow that actually has steps; fall back to oldest if none have steps
    SELECT f.* INTO v_source_flow
    FROM public.instagram_conversation_flows f
    WHERE f.settings_group_id = p_group_id
      AND EXISTS (SELECT 1 FROM public.instagram_flow_steps s WHERE s.flow_id = f.id)
    ...
    LOOP
      INSERT INTO public.instagram_flow_steps (
        ...
        v_source_step.wait_for_reply, v_source_step.wait_timeout_minutes, v_source_step.branch_type,
        v_source_step.branch_conditions, NULL
      )
