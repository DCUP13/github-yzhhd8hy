/*
# Restore autoresponder unique constraint fix in apply_settings_group_to_account

## Problem
The revert migration recreated apply_settings_group_to_account(uuid, uuid) from the
original migration, losing the fix from 20260820204144 that handles the
UNIQUE(account_id) constraint on instagram_autoresponder_settings.

## Fix
Recreate the uuid-typed function with the autoresponder fix: check for ANY existing
autoresponder setting for the target account (not just one with the same group_id),
and update it instead of trying to insert a duplicate.
*/
